...
Blockchain: Wagmi + Viem (type-safe Web3)
Wallet: RainbowKit (multi-wallet support)
Data: The Graph (historical data)
Charts: Recharts (carbon credit price charts)
Maps: Mapbox GL (task location visualization)
File Upload: Pinata SDK (IPFS)
...
│   ├── TaskBrowse.tsx             # Browse all tasks
│   ├── TaskDetail.tsx             # Single task view
│   ├── TaskCreate.tsx             # Create new task
│   ├── OperatorDashboard.tsx      # Operator tools
│   ├── ValidatorDashboard.tsx     # Validation queue
│   ├── MyCredits.tsx              # User's carbon credits
│   ├── Marketplace.tsx            # Trade credits
│   ├── PredictionMarkets.tsx      # All active markets
│   ├── Governance.tsx             # DAO proposals
│   ├── DataExplorer.tsx           # DeSci datasets
│   └── Profile.tsx                # User stats
│
├── components/
│   ├── TaskCard.tsx               # Task summary card
│   ├── FundingProgress.tsx        # Progress bar
│   ├── VerificationStatus.tsx     # Validator votes
│   ├── PredictionWidget.tsx       # Market odds display
│   ├── CreditCard.tsx             # Carbon credit display
│   ├── OrderBookTable.tsx         # Marketplace orders
│   ├── ProposalCard.tsx           # Governance proposal
│   └── WalletConnect.tsx          # RainbowKit wrapper
│
├── hooks/
│   ├── useTaskRegistry.ts         # Contract read/write
│   ├── useFundingPool.ts          # Funding operations
│   ├── useMarketplace.ts          # Trading logic
│   ├── useGovernance.ts           # DAO interactions
...
```typescript
// hooks/useTaskRegistry.ts
import { useContractRead, useContractWrite } from 'wagmi';
import { TaskRegistryABI } from '@/utils/contracts';
...
export function useCreateTask() {
  ...
  const createTask = async (
    ...
    return write({
      args: [description, estimatedCost, expectedCO2, location, deadline, proofRequirements]
    });
  };

  return { createTask, isLoading, isSuccess };
}
...
```
1. Access Control (OpenZeppelin Ownable, AccessControl):
   ✓ Only VerificationManager can finalize tasks
   ✓ Only FundingPool can update funding status
   ✓ Only Governance can change parameters
   ✓ Only assigned operator can submit proof

2. Reentrancy Protection (OpenZeppelin ReentrancyGuard):
   ✓ All functions transferring funds
   ✓ External calls to untrusted contracts
   ✓ State changes before external calls

3. Integer Overflow/Underflow:
   ✓ Solidity 0.8+ has built-in checks
   ✓ Use SafeMath patterns for complex calculations

4. Front-Running Prevention:
   ✓ Commit-reveal for sensitive operations
   ✓ Time-locks on critical state changes
...
5. Oracle Security:
   ...
   ✓ Chainlink for trusted data

6. Pause Mechanism:
   ✓ Emergency pause for all contracts
   ✓ Owner can pause during security incidents
...
□ External audit (Trail of Bits, OpenZeppelin, etc.)
...
```
Tools:
• Tenderly: Real-time transaction monitoring, alerts
• Dune Analytics: Custom dashboards for protocol metrics
• The Graph: Historical data and trends

Key Metrics:
├─ Total tasks created
├─ Total funding volume
├─ Total CO₂ offset verified
...
Alerts:
• Large stake slashes (potential fraud)
• Failed transactions (contract bugs)
• Unusual trading patterns (market manipulation)
• Governance proposals reaching quorum
• Oracle failures
• Gas price spikes
```
...
• Monitor: popular tasks, conversion funnels, bounce rates
...
| # | Edge Case | Detection | Resolution | Gas Cost | User Impact |
|---|-----------|-----------|------------|----------|-------------|
| 1 | Task underfunded for months | Timeout trigger | Auto-cancel, refund funders | Low | Minimal - money returned |
| 2 | Operator abandons mid-work | Deadline check | Auto-fail, slash stake | Low | Funders refunded |
| 3 | Validator collusion | Statistical analysis | DAO review, slash validators | High | May need governance |
| 4 | Fake GPS coordinates | Oracle cross-check | Auto-reject, slash | Medium | Prevents fraud |
| 5 | Duplicate task proposals | IPFS hash comparison | Warn proposer, allow choice | Low | User confusion |
| 6 | Credit double-spend attempt | ERC1155 native protection | Transaction reverts | Zero | Impossible |
| 7 | Market manipulation (pump/dump) | Price deviation alerts | Circuit breakers, pause | Medium | Protects traders |
| 8 | Oracle downtime | Timeout + fallback | Use cached data or manual | Low | Slight delay |
| 9 | IPFS data unavailable | Multiple pin services | Fallback to backup pins | Zero | Transparent to user |
| 10 | Governance attack (51%) | Token distribution monitoring | Time-locks, quadratic voting | Zero | Preventative |
| 11 | Operator completes 80% of work | Partial credit option | Pro-rated payment/credits | Medium | Requires validation |
| 12 | Funder wants to exit early | Status check | Allow if not started, else locked | Low | Clear terms |
| 13 | Validator never votes | Timeout mechanism | Replace validator, no reward | Low | Transparent |
| 14 | Prediction market low liquidity | Market maker incentives | Treasury seeds liquidity | Medium | Improves UX |
| 15 | Credit burned by accident | ERC1155 burn event | Irreversible (by design) | Zero | User education |
| 16 | Proposal spam | Bond requirement | Locked capital deters spam | Zero | Maintains quality |
| 17 | Model prediction always wrong | Performance tracking | Auto-disable at <50% accuracy | Low | Self-correcting |
| 18 | Treasury depleted | Revenue monitoring | Increase fees or pause features | Zero | Governance decision |
| 19 | Network congestion (high gas) | Gas price oracle | Queue transactions | Zero | Slight delay |
| 20 | Smart contract bug discovered | Pause mechanism | Emergency pause, deploy fix | High | Temporary disruption |

---

## User Experience Flows

### Optimal Happy Path (From User Perspective)

**For Funder (5-minute experience):**
```
1. Land on homepage (0:00)
   └─ See: "250 projects funded, 50,000 tons CO₂ offset"

2. Browse tasks (0:30)
   └─ Filter by: Location, Project Type, Funding Progress
   └─ See appealing task: Mangrove restoration, 80% funded

3. Click task card (1:00)
   └─ View full details, photos, timeline
   └─ See prediction market: 75% chance of success
   └─ See operator reputation: 95% success rate

4. Decide to fund (2:00)
   └─ Click "Fund Project"
   └─ Enter amount: 1,000 cUSD
   └─ See estimate: "You'll receive ~8.5 tons of carbon credits"

5. Connect wallet (2:30)
   └─ RainbowKit modal appears
   └─ Choose MetaMask
   └─ Approve connection

6. Approve cUSD (3:00)
   └─ First transaction: ERC20 approval
   └─ Sign in wallet
   └─ Wait ~5 seconds (Celo is fast)

7. Confirm funding (3:30)
   └─ Second transaction: actual funding
   └─ Sign in wallet
   └─ Wait ~5 seconds

8. Success! (4:00)
   └─ Toast notification: "Funding successful!"
   └─ Dashboard updates: "You funded 1 project"
   └─ Receive email: "Track your project at [link]"

9. Track progress (ongoing)
   └─ Return weekly to check status
   └─ Receive notifications at key milestones:
      • "Task started! Operator assigned."
      • "Under verification - 2/3 validators approved"
      • "Verified! You received 8.5 carbon credit tokens"

10. Receive credits (3 months later)
    └─ Credits appear in wallet
    └─ Options: Hold, Trade, or Retire
```

**For Operator (3-month experience):**
```
Week 1:
- Browse available funded tasks
- Assess feasibility
- Stake collateral (5,000 CELO)
- Accept task
- Plan logistics

Weeks 2-12:
- Execute environmental work
- Collect evidence daily (GPS photos)
- (Optional) Post progress updates
- Monitor deadline

Week 13:
- Compile all proof materials
- Upload to IPFS (200 MB of data)
...
- Vote completes: APPROVED

Week 15:
- Receive payment: 49,000 cUSD
- Collateral released: 5,000 CELO
- Reputation score increases
- Can immediately accept next task
```
...
- Funders want verified impact → carbon credits
- Operators want payment → must provide proof
- Validators want fees → must be accurate
- Traders want profit → provide liquidity
- Platform wants volume → takes small fees
- Everyone benefits from honest behavior

**2. Multiple Verification Layers:**
- Human validators (domain experts)
- Oracle data (objective sensors)
...
- 2% platform fee covers costs
- Marketplace fees scale with usage
- Treasury grows over time
- Self-sustaining validator economy
- No external funding needed long-term

**5. Composability:**
- Carbon credits are ERC1155 (standard)
- Integrates with DeFi (collateral, lending)
- Oracles extensible (add new data sources)
- Governance upgradeable (community-driven)
